# -*- coding: utf-8 -*-
"""FINAL FINAL FINAL DIABETES PREDICTION TRAINING.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1skFii5xhJcmj9o2d6n1EOLn1Xd8Z84kB
...
table_md += "| Model | Accuracy | Precision | Recall | F1-Score |\n"
table_md += "|-------|----------|-----------|--------|----------|\n"
for row in results:
    table_md += f"| {row[0]} | {row[1]:.4f} | {row[2]:.4f} | {row[3]:.4f} | {row[4]:.4f} |\n"
...
for name, model in models.items():
    ...
    # Store results
    results.append([
        name, accuracy_score(y_test_resampledfinal, y_pred),
        report["1"]["precision"], report["1"]["recall"], report["1"]["f1-score"]
    ])
...
for name, model in models.items():
    ...
    results_cv.append([
        name, acc_scores.mean(),
        prec_scores.mean(), recall_scores.mean(), f1_scores.mean()
    ])

df_results_cv = pd.DataFrame(results_cv, columns=["Model", "Accuracy", "Precision", "Recall", "F1-Score"])
...
df_results_cv_smote = pd.DataFrame(results_cv_smote, columns=["Model", "Accuracy", "Precision", "Recall", "F1-Score"])
...
table_md_smote += "| Model | Accuracy | Precision | Recall | F1-Score |\n"
table_md_smote += "|-------|----------|-----------|--------|----------|\n"
for row in results_cv_smote:
    table_md_smote += f"| {row[0]} | {row[1]:.4f} | {row[2]:.4f} | {row[3]:.4f} | {row[4]:.4f} |\n"
...
df_best_results = pd.DataFrame(
    ...
    columns=[
        ...
        "Best Hyperparameters", "Test Accuracy", "Test Precision", "Test Recall", "Test F1-Score"
    ]
)

table_md_best = "### Best Models Performance (After Grid Search & Final Test Evaluation)\n\n"
table_md_best += "| Model | CV Accuracy | CV Precision | CV Recall | CV F1-Score | Best Hyperparameters | Test Accuracy | Test Precision | Test Recall | Test F1-Score |\n"
table_md_best += "|-------|------------|-------------|-----------|------------|----------------------|--------------|--------------|-----------|------------|\n"

for row in best_results:
    table_md_best += f"| {row[0]} | {row[1]:.4f} | {row[2]:.4f} | {row[3]:.4f} | {row[4]:.4f} | {row[5]} | {row[6]:.4f} | {row[7]:.4f} | {row[8]:.4f} | {row[9]:.4f} |\n"
...
for name, model in best_models_smote.items():
    ...
    best_results_smote.append([
        name, cv_accuracy, acc, report["1"]["precision"], report["1"]["recall"], report["1"]["f1-score"],
...
df_best_results_smote = pd.DataFrame(
    best_results_smote,
    columns=["Model", "CV Accuracy", "Test Accuracy", "Precision", "Recall", "F1-Score", "Best Hyperparameters"]
)

table_md_best_smote = "### Best Models Performance (After Grid Search on SMOTEd Dataset)\n\n"
table_md_best_smote += "| Model | CV Accuracy | Test Accuracy | Precision | Recall | F1-Score | Best Hyperparameters |\n"
table_md_best_smote += "|-------|------------|---------------|-----------|--------|----------|----------------------|\n"

for row in best_results_smote:
    table_md_best_smote += f"| {row[0]} | {row[1]:.4f} | {row[2]:.4f} | {row[3]:.4f} | {row[4]:.4f} | {row[5]:.4f} | {row[6]} |\n"
