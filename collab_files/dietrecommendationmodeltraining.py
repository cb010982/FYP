# -*- coding: utf-8 -*-
"""FINAL VERSION of FINALISED FINAL FINAL FINAL MEAL RECOMMENDATION TRAINING.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1T7zkt8v9Bl5ejM4BUs0en7xup703qZME
"""
...
# Convert ratings to binary values to show like and dislike rather rating 4 and 5
...
results = {
    ...
    "MAE": [0.485077, 0.477714, 0.5008, 0.4993],
    "RMSE": [0.507835, 0.541156, 0.5189, 0.5210],
    "MSE": [ 0.257896, 0.292850, 0.2693, 0.2714]
}
...
def evaluate_final_model(embeddings, model, model_name):
    ...
    predicted = predicted_ratings.numpy()
...
pre_cv_mse = [0.257896, 0.292850, 0.269300, 0.271400]  # Before Cross-Validation
post_cv_mse = [0.259557, 0.308078, 0.2252, 0.2106]  # After Cross-Validation
...
#  Retrain final model using best hyperparams on full data
print("\n Retraining best NFM model on full dataset...")
nfm_embeddings = create_embeddings(num_users, num_items, best_nfm_params["embed_dim"])
nfm_model = create_model(best_nfm_params["embed_dim"], best_nfm_params["deep_layers"], interaction_type="element-wise")

optimizer = optim.Adam(
    list(nfm_embeddings["user_embedding"].parameters()) +
    list(nfm_embeddings["item_embedding"].parameters()) +
    list(nfm_model["fm_layer"].parameters()) +
    list(nfm_model["dnn"].parameters()),
    lr=best_nfm_params["learning_rate"]
)

criterion = nn.BCELoss()
for epoch in range(best_nfm_params["num_epochs"]):
    optimizer.zero_grad()
    outputs = forward(features[:, 0], features[:, 1], nfm_embeddings, nfm_model)
    loss = criterion(outputs, labels)
    loss.backward()
    optimizer.step()

#  Save the correct weights now
...
print(" Saved model weights with best hyperparameters")
...
# Evaluate best trained model on test set
with torch.no_grad():
    predictions = forward(test_tensor[:, 0], test_tensor[:, 1], nfm_embeddings, nfm_model)
...
rmse_values = [ 0.4570722280637526, 0.41944847270604546, 0.7047053658951145, 0.5658868061864143]
mse_values = [0.210532808303833, 0.182038377225399, 0.49660965272136715 , 0.3202278774158604]
mae_values = [0.40069614052772523, 0.34188335239887235, 0.5067960107386278, 0.481279047831659]
